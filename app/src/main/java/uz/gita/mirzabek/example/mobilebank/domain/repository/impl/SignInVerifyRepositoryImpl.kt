...
@Singleton
class SignInVerifyRepositoryImpl @Inject constructor(
    private val authApi: AuthApi,
    private val pref:AppPref,
    private val gson:Gson

) :SignInVerifyRepository{
    override fun verifySignIn(code: String): Flow<Result<Unit>> = flow{
        val response=authApi.verifySignIn(pref.token,VerifySigInRequest(code))
        ...
        when(response.code()){
            in 200..299->{
                ...
                val errorMessage=gson.fromJson(response.errorBody()?.string().toString(),MessageResponse::class.java)
